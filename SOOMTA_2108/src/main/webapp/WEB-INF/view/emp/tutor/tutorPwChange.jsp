...
<body>
	...
	<!-- 중앙 -->
	<div class="main">
	<form:form action="tutorPwChangeOk" method="post" name="frm" onsubmit="return funcCon()" modelAttribute="tutorCommand"> 
	
		<table>
		<thead>
			<tr
				style="background-color: #0F4C81; color: white; font-size: 25px; padding: 10px">
				<th colspan="2">'${tutorCommand.tutorId }' 비밀번호 변경 
				</th>
			</tr></thead>
								<tbody>
						<tr class="first_tr">
							<th>아이디</th>
							<td><form:input path="tutorId" readonly="true" class="id"/></td></tr>
						<tr>
							<th>변경 비밀번호</th>
							<td><input type="password" name="tutorPw"
								size="30" required
						...
						<tr>
							<th>변경 비밀번호 확인</th>
							<td><input type="password" name="tutorPwCon" required
								size="30"
								pattern="^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@$!%*#?&^])[A-Za-z\d$@$!%*#?&^]{8,15}$"
								class="pw" id="pw2" onkeyup="check_pw()"/>
	&nbsp;<span id="check"></span>&nbsp;<span class="tutorPwConErr"><form:errors path="tutorPwCon"/></span></td>
						</tr>
					</tbody>
			<tfoot>
			<tr>
					...
					onclick="location.href='tutorInfo?tutorId=${tutor.tutorId}'" /></div>
			</td></tr></tfoot>
		</table>
		</form:form>
	</div>
